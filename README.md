...
❯ Pipeline             download → mark → crop → package → narration → review → tts → mix → render
  ...
  Run & check          run the pipeline, see a chapter's status, verify outputs
...
**Hands-off:** `./run.sh auto -p my_manga --chapters 1-5` takes chapters from download to rendered video, and you only do the Gemini hand-offs - it lists each upload, watches `llm_crops.json`, `narration.json` and `memory.json`, and carries on by itself the moment a reply is saved (fix requests included). See [docs/llm_crop_guide.md](docs/llm_crop_guide.md#hands-off-auto).
