...
Upload `pages.zip` and `prompts/crop_generation_prompt.md` to your multimodal LLM (e.g. Claude 3.5 Sonnet, GPT-4o, Gemini 1.5 Pro). Save the resulting JSON to:
```text
...
Upload your generated vision archive (`sheets.zip` or `panels.zip`) and `prompts/narration_generation_prompt.md` to your LLM. Save the resulting script to:
```text
...
├── prompts/
│   ├── crop_generation_prompt.md       # Master coordinate crop extraction prompt
│   └── narration_generation_prompt.md  # Master objective scriptwriter prompt
├── projects/
│   └── <project_name>/
