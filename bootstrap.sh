...
# Chatterbox's environment (.tools/venv-chatterbox) is named in remanga/tool_envs/catalog.py.
...
# ---------------------------------------------------------------------------
# A lightweight main env, plus Chatterbox's under .tools/ so torch never touches
# the main one. The main env runs `.tools/venv-chatterbox/bin/python` as a
...
say "Provisioning Chatterbox Turbo's environment..."
try_step "Chatterbox environment provisioning" \
...
say "Downloading the Chatterbox Turbo weights..."
try_step "Chatterbox weight download" "$VENV_DIR/bin/python3" -m remanga.cli setup
