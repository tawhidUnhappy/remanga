...
Defined together here and spliced into the Chapter Production and
Project-wide lists right after `mark` and `mark-all`, so the workflow's whole
command surface is one file, and the menus list it next to the step it
replaces."""
...
from remanga.commands.setup_rows import LLM_CROP_SETUP
...
LLM_CROP_CHAPTER_COMMANDS: list[Command] = [
    Command(
        "crop-grid",
...
LLM_CROP_PROJECT_COMMANDS: list[Command] = [
    Command(
        "crop-grid-all",
        ...
        project_handlers.crop_grid_all,
        [
            project_param(),
