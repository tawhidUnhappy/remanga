"""Chapters: how they are named, found, ordered and selected.

Depends on nothing but paths, so anything can import it without dragging in
the audio/video stack."""
...
def expand_chapter_selection(raw: str, available: Sequence[str], *, strict: bool = False) -> list[str]:
    ...
    actually act on (a project's folders, or MangaDex's listing for a
    download) - so '1-9999' can't manufacture chapters that don't exist.
...
def fmt_chapter(chapter_num) -> str:
    """A plain integer chapter zero-padded ("7" -> "007"); anything else
    ("10.5", a special chapter's label) as it is."""
    s = str(chapter_num).strip()
    return s.zfill(CHAPTER_WIDTH) if s.isdigit() else s


def page_stem(chapter_num, page_index: int) -> str:
    """A downloaded page's file name without its extension: "001_007".
    Zero-padded so a plain directory listing is reading order."""
    return f"{fmt_chapter(chapter_num)}_{str(int(page_index)).zfill(PAGE_WIDTH)}"
