...
from remanga.narration import read_reply
...
class ReviewerState(PanelPassState):
    ...
    def __init__(self, chapter_dir: Path, chapter_num: str):
        ...
        # The narration as prompts/narration.md asks for it: one entry per
        # panel inside a `narration` section, with `memory` beside it. Read
        # through narration.read_reply, so whatever an LLM wrapped it in is
        # the reviewer's problem in exactly one place.
        doc, _memory = read_reply(narration_path)
        self.chapter_label = str(doc.get("chapter", chapter_num))
        self.narration_entries: list[dict[str, Any]] = [
            {"panel_id": entry.get("panel"), "text": entry.get("text", ""), "skip": entry.get("skip", "")}
            for entry in doc.get("panels", []) if isinstance(entry, dict) and entry.get("panel")
        ]
