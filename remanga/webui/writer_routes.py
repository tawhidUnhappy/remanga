...
def create_writer_app(state: WriterState, config: WriterConfig, project_name: str) -> Flask:
    ...
    def finish():
        ...
        # The document's shape is prompts/narration.md's: panels live under
        # its `narration` section, and an untyped one is a skip.
        panels = narration["narration"]["panels"]
        written = sum(1 for entry in panels if (entry.get("text") or "").strip())
        console.print(
            f"[bold green]✓ narration.json saved[/] "
            f"({written}/{len(panels)} panel(s) with text) to: {_esc(str(state.narration_path))}"
        )
        state.submitted = True
        state.finished.set()
        return jsonify({"ok": True, "total_panels": len(panels), "written": written})
