...
reason as every isolated-venv worker.

Chatterbox Turbo (ResembleAI/chatterbox-turbo, 350M, MIT) clones a narrator
from a reference recording instead of shipping named voices, so its
requests carry a clip PATH. Turning that
...
rather than once per page: the conditionals are kept, and rebuilt only when
a request names a different file, or the same file after it changed on disk.

Every page is generated from the same fixed seed. Turbo samples
(temperature 0.8), so without one, re-running a page - which resume does to
the pages either side of an interruption - comes back as a different take
...
There is no speed field: the clip is used at the model's own pace.

Every model call runs with stdout redirected to a
...
# One seed for every page - see the module docstring.
SEED = 0
